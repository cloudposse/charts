...
The above command sets the host parameter to `test.com` and configures the VPN at that end-point.

Alternatively, a YAML file that specifies the values for the above parameters can be provided when installing the chart for the first time. For example,
